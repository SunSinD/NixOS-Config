  ...
  flake.nixosModules.shell = { ... }: {
    home-manager.users.SunSD = { ... }: {
      ...
      programs.bash = {
        ...
        shellAliases = {
          rebuild = "sudo nixos-rebuild switch --flake ~/nixconf#$(hostname) > /tmp/_build.log 2>&1 && echo '  OK: Rebuilt.' || (echo '  ERROR: Rebuild failed.' && grep -i error /tmp/_build.log | head -5)";
